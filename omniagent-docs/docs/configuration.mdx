...
* **`max_delegation_depth`**: Max nesting depth for [agent delegation](/docs/agent/delegation) chains before raising `OmniAgent::MaxDelegationDepthError`. Defaults to `5`.
...
See [Supported Providers](/docs/supported-providers) for provider-specific setup. The `:mock` provider is useful for tests — it returns a canned response without making network calls.
